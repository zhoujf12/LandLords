...
void GamePanel::initCardMap()
{
    ...
    for(int i = 0, suit = Card::Suit_Begin+1;suit<Card::Suit_End;++i,++suit){
        for(int j = 0, pt = Card::Card_Begin+1;pt<Card::Card_SJ;++j,++pt){
...
void GamePanel::gameStatusPrecess(GameControl::GameStatus status)
{
    ...
    case GameControl::PlayingHand:
        ...
        for(int i = 0;i<m_playerList.size();++i)
        {
            ...
            QPixmap pixmap = loadRoleImage(player->getSex(),player->getDireaction(),player->getRole());
...
void GamePanel::cardMoveStep(Player* player,int curPos)
{
    ...
    QPoint pos[] = {
        ...
        QPoint(m_baseCardPos.x() ,m_baseCardPos.y()+curPos*unit[2])//当前玩家
    };
...
QPixmap GamePanel::loadRoleImage(Player::Sex sex, Player::Direaction direct, Player::Role role)
{
    // 找图片
